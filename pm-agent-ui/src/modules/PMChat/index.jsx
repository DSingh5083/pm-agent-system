...
// Two tabs: Chat + Stakeholder Translator
// Shared project/feature context selector at top.
...
function ContextSelector({ ps, selectedProject, selectedFeature, onProjectChange, onFeatureChange }) {
  ...
  return (
    <div style={{ padding: "10px 18px", background: "#fff", borderBottom: "1px solid #e8e8e8", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
      ...
      <select value={selectedProject?.id || ""} onChange={e => handleProjectChange(e.target.value)}
      ...
      {selectedProject && features.length > 0 && (
        <>
          ...
          <select value={selectedFeature?.id || ""} onChange={e => handleFeatureChange(e.target.value)}
            style={{ padding: "5px 10px", borderRadius: 8, border: "1px solid #e0e0e0", fontSize: 12, color: "#1a1a2a", background: "#f5f6f8", cursor: "pointer", outline: "none", fontFamily: "inherit" }}>
        ...
        {!activeProject ? (
          ...
          <>
            <div style={{ padding: "7px 9px", background: "#0066FF08", border: "1px solid #0066FF20", borderRadius: 8, marginBottom: 8 }}>
              ...
              {activeProject.description && <div style={{ fontSize: 10, color: "#666", lineHeight: 1.4 }}>{activeProject.description.slice(0, 80)}...</div>}
            </div>
            ...
            {Object.entries(PROJECT_STAGE_LABELS).map(([id, label]) => {
              ...
              return (
                <div key={id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 5px", borderRadius: 4, marginBottom: 2 }}>
                  <span style={{ fontSize: 10, color: has ? "#00AA44" : "#ccc" }}>{has ? "✓" : "–"}</span>
            ...
            {(projectLoaded.length + featureLoaded.length) > 0 && (
              <div style={{ marginTop: 10, padding: "7px 9px", background: "#00AA4408", border: "1px solid #00AA4420", borderRadius: 6 }}>
                <div style={{ fontSize: 11, color: "#00AA44", fontWeight: 600 }}>✓ {projectLoaded.length + featureLoaded.length} stages in context</div>
              </div>
            )}
          </>
        )}
...
function MessageBubble({ message }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: isUser ? "flex-end" : "flex-start", marginBottom: 14, alignItems: "flex-end", gap: 8 }}>
      {!isUser && <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, background: "linear-gradient(135deg, #0066FF, #8B00FF)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#fff", fontWeight: 800 }}>P</div>}
      ...
      {isUser && <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, background: "#1a1a2a", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: "#fff", fontWeight: 800 }}>Y</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 14 }}>
      ...
      <div style={{ background: "#fff", borderRadius: "16px 16px 16px 4px", padding: "12px 16px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)", border: "1px solid #f0f0f0", display: "flex", gap: 4, alignItems: "center" }}>
        {[0, 1, 2].map(i => <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: "#0066FF", animation: `bounce 1.2s ease-in-out ${i * 0.2}s infinite` }} />)}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "10px 18px", background: "#fff", borderBottom: "1px solid #e8e8e8", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          ...
          {chatMessages.length > 0 && (
            <button onClick={clearChat} style={{ padding: "5px 10px", background: "none", border: "1px solid #e8e8e8", borderRadius: 6, fontSize: 11, color: "#999", cursor: "pointer" }}>Clear</button>
          )}
        </div>
            ...
            <div style={{ maxWidth: 540, margin: "0 auto" }}>
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                ...
                <div style={{ fontSize: 12, color: "#999", lineHeight: 1.6 }}>
                  {contextCount > 0 ? `${contextCount} pipeline stages loaded. Ask me anything specific to this project.` : "Select a project above, or just ask me anything."}
                </div>
              </div>
        ...
        <div style={{ padding: "12px 18px", background: "#fff", borderTop: "1px solid #e8e8e8", flexShrink: 0 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
            ...
            <button onClick={() => sendMessage()} disabled={loading || !input.trim()}
              style={{ width: 40, height: 40, flexShrink: 0, borderRadius: "50%", background: loading || !input.trim() ? "#e8e8e8" : "linear-gradient(135deg, #0066FF, #8B00FF)", border: "none", cursor: loading || !input.trim() ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, color: loading || !input.trim() ? "#bbb" : "#fff", transition: "all 0.2s" }}>↑</button>
          </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 24, background: "#f5f6f8" }}>
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: "#888" }}>
            {Object.keys(results).length > 0 ? `${Object.keys(results).length} generated · click any card to expand` : "Generate a tailored message for each stakeholder"}
          </div>
        </div>
        {STAKEHOLDERS.map(s => (
